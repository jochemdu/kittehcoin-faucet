...
function simple_php_captcha($config = array()) {
	...
	$image_src = substr(__FILE__, strlen($_SERVER['DOCUMENT_ROOT'])) . '?_CAPTCHA&amp;s='.urlencode($captcha_config["session_name"]).'&amp;t=' . urlencode(microtime()); // jonn4y
...
if( isset($_GET['_CAPTCHA']) ) {
	...
	@session_name(isset($_GET["s"])?$_GET["s"]:'sphpc'); // jonn4y
	@session_start();
	
	$captcha_config = @unserialize($_SESSION['_CAPTCHA']['config']); // jonn4y
